...
res = bullet_simulation.getLinkState(sim.robot,sim.jointIds['LARM_JOINT5'],computeLinkVelocity=1,computeForwardKinematics=1)
link_trn, link_rot, com_trn, com_rot, frame_pos, frame_rot, link_vt, link_vr = res
j_geo, j_rot = bullet_simulation.calculateJacobian(sim.robot,sim.jointIds['LARM_JOINT5'],com_trn,mpos,[0.0] * len(mpos), [0.0] * len(mpos),)
jPos = sim.jacobianMatrix('LARM_JOINT5')
